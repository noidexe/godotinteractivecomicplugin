# Widget for Godot's 2d viewport. Adds 4 draggable anchors to Ballon node
# The user can click and drag these anchors to resize a Balloon node around
# its center.
# Based on https://github.com/GDQuest/godot-open-rpg/blob/master/godot/addons/rect_extents_gizmo/plugin.gd
# by Nathan Lovato (GDQuest)
tool
...
var rect_drag_start: Dictionary = {'size': Vector2()}
...
func forward_canvas_draw_over_viewport(overlay: Control) -> void:
	...
	var half_size: Vector2 = rect_extents.scale * rect_extents.size / 2.0
	var edit_anchors := {
		Anchors.TOP_LEFT: pos - half_size,
		Anchors.TOP_RIGHT: pos + Vector2(half_size.x, -1.0 * half_size.y),
		Anchors.BOTTOM_LEFT: pos + Vector2(-1.0 * half_size.x, half_size.y),
	...
	for i in edit_anchors.keys():
		var anchor_center: Vector2 = transform_viewport * (transform_global * edit_anchors[i])
		var new_anchor = {
			'position': anchor_center,
			'rect': Rect2(anchor_center - anchor_size / 2.0, anchor_size),
		}
		draw_anchor(new_anchor, overlay, i)
...
func draw_anchor(anchor: Dictionary, overlay: Control, type: int) -> void:
	var pos = anchor['position']
	match type:
		Anchors.TOP_LEFT:
			overlay.draw_line(pos, pos + Vector2.RIGHT * CIRCLE_RADIUS, STROKE_COLOR,2)
			overlay.draw_line(pos, pos + Vector2.DOWN * CIRCLE_RADIUS, STROKE_COLOR,2)
		Anchors.TOP_RIGHT:
			overlay.draw_line(pos, pos + Vector2.LEFT * CIRCLE_RADIUS, STROKE_COLOR,2)
			overlay.draw_line(pos, pos + Vector2.DOWN * CIRCLE_RADIUS, STROKE_COLOR,2)
		Anchors.BOTTOM_LEFT:
			overlay.draw_line(pos, pos + Vector2.RIGHT * CIRCLE_RADIUS, STROKE_COLOR,2)
			overlay.draw_line(pos, pos + Vector2.UP * CIRCLE_RADIUS, STROKE_COLOR,2)
